...
$selectedAnswers = isset($_SESSION['answers']) ? $_SESSION['answers'] : [];

foreach ($questions as $index => $question) {
    echo "<p>{$question}</p>";
    $selectedAnswerIndex = isset($selectedAnswers[$index]) ? $selectedAnswers[$index] : null;
    foreach ($answerChoices[$index] as $key => $choice) {
        if ($key === $selectedAnswerIndex) {
            echo '<h3 style="color:red;">'. $choice . '</h3>';
        }
    }
    echo "<br>";
}
